...
#define DMA_MODE
    ...
    if(!USBHandleBusy(CDCDataOutHandle))
    {
         ...
         */
#ifdef DMA_MODE    //DMA Transfer
        DMADST5 = ((unsigned int)buffer);             //Setup destination pointer
        DMASRC5 = ((unsigned int)&cdc_data_rx);       //Setup source pointer
        DMACNT5 = (len + 1) >> 1;                     //Setup the number of bytes to send

        DMACH5bits.CHREQ = 1;
        DMACH5bits.CHEN = 1;                          //Enable channel  
